    ...
    public static function connect(): void {
        ...
        try { 
            ...
            if ($mysqli && !$mysqli->connect_errno) { $mysqli->set_charset('utf8mb4'); self::$conn = $mysqli; }
                ...
                try { @file_put_contents((defined('LOG_DIR')?LOG_DIR:__DIR__) . '/db_connect_error.log', '[' . date('c') . "] errno=" . ($mysqli ? $mysqli->connect_errno : 'n/a') . " msg=" . ($mysqli ? $mysqli->connect_error : 'n/a') . " host=" . DB_HOST . " user=" . DB_USER . " name=" . DB_NAME . "\n", FILE_APPEND); } catch (\Throwable $e) {}
            }
